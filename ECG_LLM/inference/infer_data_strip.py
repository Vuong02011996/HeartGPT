...
from ECG_LLM.process_data_training.helper import ecg_to_tokens
...
def get_peak_symbols():
    ...
    for event_path in tqdm(all_events):
        # if "65beb4ddc69788a1bdb71b0b" not in event_path:
        #     continue
        ...
        if ((stopSample - startSample) < (10 * SAMPLING_RATE) or
            ...
            if len(beats_predict) == 0 or len(beats) == 0:
                error += 1
            ...
            if len(labels_gt) != len(symbols_predict):
                print(event_path)
                print("len(labels_gt) != len(symbols_predict)")
                print("beats_predict: ", beats_predict)
                print("beats_gt: ", beats_gt)
                error += 1
                with open(error_log_file, 'a') as f:
                    f.write(f"{event_path}\n")
                continue

            if len(index) > 0 or all(labels_gt == symbols_predict) is False:
                print(event_path)
                print("beats_predict: ", beats_predict)
                print("beats_gt: ", beats_gt)
                error += 1
...
if __name__ == '__main__':
    ...
    # Check if the error log file exists and delete it
