...
def del_result_fda(fda_directory, output_ec57_directory):
    curr_dir = os.path.dirname(os.path.abspath(__file__))
...
def ec57_eval(dir_db,
    ...
    file_names = glob(physionet_directory + dir_db + '/1.0.0/*.dat')
    ...
    if beat_ext_ai is not None:
        os.makedirs(output_ec57_directory + dir_db + half_ext + '/1.0.0', exist_ok=True)
        bxb_script(curr_dir + '/bxb-script.sh',
                   # physionet_directory + dir_db + '/1.0.0',
                   physionet_directory + dir_db,
                   output_ec57_directory + dir_db + half_ext + '/1.0.0',
                   beat_ext_db,
                   beat_ext_ai,
...
def ec57_eval_fda(output_ec57_directory,
              fda_directory,
              beat_ext_db,
              event_ext_db,
              beat_ext_ai,
    ...
    file_names = glob(fda_directory + '/*.dat')
    file_names = [p[:-4] for p in file_names]
    file_names = np.sort(file_names)
    header = wf.rdheader(file_names[0])
    fs_origin = header.fs
    ...
    if beat_ext_ai is not None:
        bxb_script(curr_dir + '/bxb-script-event.sh',
                   fda_directory,
                   output_ec57_directory,
                   beat_ext_db,
                   beat_ext_ai,
                   'FDA_QRS_report_line',
                   'FDA_QRS_report_standard')
        rann_script(curr_dir + '/rdann-script-event.sh',
                    fda_directory,
                    beat_ext_db,
                    beat_ext_ai)
        ...
        mxm_script(curr_dir + '/mxm-script-event.sh',
                   fda_directory,
                   output_ec57_directory,
                   'rr' + beat_ext_db,
                   'rr' + beat_ext_ai,
    ...
    if event_ext_ai is not None:
        epicmp_script(curr_dir + '/epicmp-script-event.sh',
                      fda_directory,
                      output_ec57_directory,
                      'FDA_AFib_report',
                      event_ext_db,
...
def ec57_evallabel2(dir_db,
              output_ec57_labels2_directory,
              label2_directory,
              beat_ext_db,
              event_ext_db,
              beat_ext_ai,
    ...
    :param dir_db:
    :param output_ec57_labels2_directory:
    :param label2_directory:
    :param beat_ext_db:
    :param event_ext_db:
    :param beat_ext_ai:
    :param event_ext_ai:
    ...
    if not isdir(output_ec57_labels2_directory):
        os.makedirs(output_ec57_labels2_directory)
