...
model_path_finetune = "/home/server2/Desktop/Vuong/Reference_Project/HeartGPT/Model_files/HeartGPT_finetune_beat_mitdb.pth"
...
for file in train_records:
    signal, labels = re_process_data(file)
    all_signals.append(signal)
    all_labels.append(labels)

# Concatenate all signals and labels into one array
X_shuffled = np.concatenate(all_signals)
y_shuffled = np.concatenate(all_labels)
# X_shuffled, y_shuffled = re_process_data(train_records[0])
...
testX = torch.tensor(testX, dtype=torch.long)
...
def get_batch_beat(split):
    ...
    y = torch.stack([datay[i:i+block_size] for i in ix])
...
class Heart_GPT_FineTune(nn.Module):
    ...
    def __init__(self):
        ...
        self.blocks = nn.Sequential(*[Block(n_embd, n_head = n_head) for _ in range(n_layer - 1)] + [Block(n_embd, n_head = n_head, mask=False)])
